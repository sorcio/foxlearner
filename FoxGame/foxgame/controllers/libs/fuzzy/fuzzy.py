...
class Set(object):
    ...
    def __str__(self):
        return '{' + ', '.join('%s/%g' % (str(['%g' % f for f in x]), u_x)
                                          for x, u_x in self) + '}'
...
class Engine(object):
    ...
    A Fuzzy Engine is composed of:
     - a collection of fuzzy variables 'self.variables';
    ...
    def _parse_condition(self, sconds):
        ...
        ant, cons = sconds.strip().split(' IS ')
        return (ant, self.variables[ant][cons])
    ...
    def _parse_rule(self, srule):
        ...
        if ' IS ' not in antecedent or ' IS ' not in consequent:
            raise SyntaxError('Malformed rule.')
    ...
    def add_rule(self, srule):
        ...
        """
        self.rules.append(self._parse_rule(srule))
