...
class TestFuzzyEngine(TestCase):
    ...
    def setUp(self):
        """
        Set up a fuzzy air-conditioning, from TestFuzzyRules class
        """
        # fuzzy temperatures
        self.cold = fuzzy.Set(None, 'cold', 'triangle', 0, 0, 17.5)
        self.cool = fuzzy.Set(None, 'cool', 'triangle', 12.5, 17.5, 22.5)
        ...
        self.engine = fuzzy.Engine([self.temperature, self.speed])
    ...
    def test_parserule(self):
        ...
        self.engine.add_rule('IF temperature IS cool THEN speed IS high')

        self.assertEqual(self.engine.evaluate(temperature=17.5)['speed'],
                         self.high)
        self.assertFalse(self.engine.evaluate(temperature=0)['speed'])
